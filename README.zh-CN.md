...
从 [v1.0.0 Release](../../releases/tag/v1.0.0) 下载 `MechrevoCustomOSD-v1.0.0-OneClickInstaller.exe`，核对 SHA256 后运行。安装器会请求一次管理员权限，保存并停用官方 `BLDHotKeyService`，避免官方与自定义 OSD 同时运行造成冲突，但不删除任何官方文件；安装后的 OSD 登录任务明确使用 `RunLevel Limited`。
